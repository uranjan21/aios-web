...
class FinancePendingTransaction(SQLModel, table=True):
    ...
    # Gmail message id the row was parsed from — the idempotency key for ingestion.
    source_email_id: Optional[str] = Field(default=None, index=True)
    # Full email body text (bank senders only) kept for audit + re-parsing on template drift.
    raw_text: Optional[str] = Field(default=None, sa_column=Column(Text))
    # Which bank parser produced this row ("hdfc", "axis", … or "llm" for the fallback).
