...
async def _adjust_balance(session, account_id: Optional[uuid.UUID], delta: float) -> None:
...
async def post_due_recurring() -> int:
    ...
    async with AsyncSessionLocal() as session:
        bills = (await session.execute(
            select(FinanceBill).where(FinanceBill.is_active == True)
        )).scalars().all()
        loans = (await session.execute(
            select(FinanceLoan).where(FinanceLoan.is_active == True)
        )).scalars().all()
        ...
        for bill in bills:
            ...
            await _adjust_balance(session, bill.account_id, -float(bill.amount))
        ...
        for loan in loans:
            ...
            await _adjust_balance(session, loan.account_id, -float(loan.emi_amount))
    ...
    for ev in events:
        await _broadcast_agent({"type": "recurring_posted", **ev})

    if posted:
        from app.services.finance.budget_alerts import check_budget_alerts
        await check_budget_alerts()  # recurring posts may push categories over thresholds
        logger.info("Recurring engine posted %d expense(s) for %s", posted, period)
    return posted


async def notify_due_tomorrow() -> int:
    ...
    async with AsyncSessionLocal() as session:
        bills = (await session.execute(
            select(FinanceBill).where(FinanceBill.is_active == True, FinanceBill.due_day == tomorrow.day)
        )).scalars().all()
        loans = (await session.execute(
            select(FinanceLoan).where(FinanceLoan.is_active == True, FinanceLoan.emi_day == tomorrow.day)
        )).scalars().all()

    for bill in bills:
        if bill.last_posted_period == period:
            continue
        await send_push_to_all("Bill due tomorrow", f"{bill.name} — ₹{float(bill.amount):,.0f}", "/areas/finance")
        notified += 1
    for loan in loans:
        if loan.last_posted_period == period:
            continue
        await send_push_to_all("EMI due tomorrow", f"{loan.name} — ₹{float(loan.emi_amount):,.0f}", "/areas/finance")
        notified += 1
