...
async def run_auto_commit_pending_transactions() -> None:
            ...
            for user in users:
                try:
                    ...
                    if pending_txs:
                        ...
                        logger.info("Auto-committed %d pending transactions for user %s", len(pending_txs), user.id)
                        
                except Exception as e:
                    logger.error("Failed to auto-commit transactions for user %s: %s", user.id, e)
                    
    except Exception as e:
        logger.error("Global job run_auto_commit_pending_transactions failed: %s", e)
