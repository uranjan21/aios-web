...
│           └── components/    # ui/ (AreaTabs…), layout/ (AreaSettingsPage, PageLayout, PageDivider),
...
- **`PageHeader` is a solid card and owns its own surface** (2026-08-03). Card background, hairline border, `elevation[1]`, `radii.md`, `spacing[5] spacing[6]` padding — icon chip (`radii.sm`, tinted by the `tone` prop) · title · meta line · actions right. The glass shell + domain radial wash that `PageContent` used to wrap it in are gone; the domain colour survives only as the chip tint. The meta line is the `subtitle` prop and sits UNDER the title, visible at every width (`PageIdentity.eyebrow` became `PageIdentity.subtitle` — the area name reads as meta, not as an uppercase eyebrow). Below `sm` the actions take their own row. Do not re-dress the header from a call site: change the component.
...
- **One card corner: `theme.radii.md`.** The page header, `Card`, `KpiCard`, `StatCard`, `ChartCard`, `InsightCard`, `DataTable` and every `ModuleGrid` tile all render at the same corner so a page reads as one material (unified 2026-08-03 — five of them were a step larger). `radii.lg`+ is for overlay chrome (popovers, menus, chat bubbles, dialogs), never a card or KPI. **Cite the token, not a pixel figure:** this line said "16px" long after the 2026-08-06 minimal-corners pass dropped the whole scale to 2/4/6/8/10/12, which makes `radii.md` **6px**. `packages/ui/src/theme/tokens.ts` is the only place a number is correct.
...
- **Multi-option toggle/filter style (user-confirmed favorite, always use this for "All / Over / Near / On track"-style filters)**: `@ledgr/ui`'s `SegmentedControl` — light `theme.color.muted` track, white/card active segment with `theme.elevation[1]`, `theme.radii.md` corners (not pill-shaped). Do NOT use a `Select` dropdown for this pattern unless the option list is long (5+) or doesn't need every option visible at once — short status/range filters (≤4 options) should be `SegmentedControl`, not a dropdown.
- **Nothing page-scoped goes in the TopBar.** The TopBar is permanent app chrome — breadcrumbs, search, assistant, theme, notifications, account. A page's own Settings link or filter dropdown does not belong in it. It briefly did (2026-08-02, when `PageHeader` was retired app-wide); reverted the same day on Utsav's call. `usePageHeaderActions()` is consumed by `PageContent` in `@ct/shared`, which renders `PageHeader` **inside the page's own content column** — title from the nav tree (published by `AppShell` through `PageIdentityProvider`, so no page hand-writes a section→label map), actions from the portal.
- **The page header currently renders on NO page** (verified 2026-08-16). `PageContent` shows it only when something has been portalled, and **`HeaderActionPortal` has zero call sites** — the controls that used to justify it became nav destinations of their own (per-area Settings is a nav entry since 2026-08-03; see the comment in `apps/finance/src/pages/FinancePage.tsx`). `PageHeader` itself is rendered only by `PageLayout.tsx` and `AreaSettingsPage.tsx`, never imported by an app. So the mechanism is intact and unused: it is what you reach for if a genuinely page-scoped control appears, not a thing to go wire up because the docs describe it. Do not "restore" a header to a page on the strength of this section.
...
- **Toolbar/action-row controls share a 32px height contract** — every interactive control that sits in a `Card` action row or `AreaToolbar` must render at exactly **32px** so they align on one baseline. Fixed at the ledgr-ui source (all `border-box`): `Button size="sm"` = 32px, `SegmentedControl size="sm"` = 32px (explicit `height` on `Root`), **`IconButton size="sm"` = 32×32px — reach for it for any icon-only control** (`md` = 36px matches `Button`); its `aria-label` is required by the type, which is the defect every hand-rolled copy shared. `DateNavBtn` and icon-only `ToolbarIconBtn` are deprecated in its favour. `ToolbarIconBtn` = `height:32px; padding:0 16px` (do NOT use vertical padding — it made it 37.5px and pushed it above the others). Never paper over a height mismatch with an inline `style={{ height: 32 }}` on the call site — fix the primitive. When adding a new toolbar control, measure it against 32px.
- **Per-area Settings is a nav destination, not a button.** Finance/Health/Career Setup are entries in `apps/shell/src/config/navigation.ts` and are reached from the sidebar; no area page renders a Settings control any more (2026-08-03). The old spec here — outline/sm button with a 14px gear, portalled through `HeaderActionPortal` — described a button that no longer exists on any page. If an area ever needs one back, it goes through the portal in that shape.
...
- **`WorkspaceLayout` is deleted** (removed with the module-kit conversion; confirmed gone 2026-08-16). Its two rules went with it: the "inputs/forms in the right 300px sticky rail, analytics in the centre" split, and the `Main` `gap: 24px` / `margin-top` doubling gotcha. Forms live in a `Dialog` and content is a `ModuleSpec[]` handed to `ModuleGrid` — there is no rail to put anything in.
