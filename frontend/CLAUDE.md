...
- **One card corner: `theme.radii.md` (16px).** The page header, `Card`, `KpiCard`, `StatCard`, `ChartCard`, `InsightCard`, `DataTable` and every `ModuleGrid` tile all render at 16px so a page reads as one material (unified 2026-08-03 — the five above were at `lg`/20px). `radii.lg`+ is for overlay chrome (popovers, menus, chat bubbles, dialogs), never a card or KPI.
