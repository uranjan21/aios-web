...
## 2026-07-23 — claude-code (Finance OS: email ingestion + monthly views)
- Shipped: **Finance OS — bank/CC email ingestion as an extension of the existing Finance module** (Postgres+Alembic, NOT Supabase — repo has none). (P0) Migration `f001_finance_email_ingestion` **merges the two open alembic heads** (`u002_failed_webhooks` + `p001_finance_composite_indexes`) into one, and adds `source_email_id`/`raw_text`/`parser` to `finance_pending_transactions` (unique `(user_id, source_email_id)` for idempotent ingest), new tables `finance_merchant_rules`/`finance_cc_bills`/`finance_obligation_payments`, and `finance_investments.committed_monthly`. (P1) Deterministic bank parsers `services/finance/email_ingest/` — shared amount/date helpers + `generic.py` extraction, thin adapters for HDFC/Axis/ICICI/SBI/CRED, sender allowlist/dispatch; **35 unit tests** in `tests/test_email_parsers.py` (verified green directly — conftest can't load offline: tiktoken download blocked by sandbox proxy). (P2) `gmail_fetch.py` full-body fetch scoped to bank senders (read-only) + `runner.py` (parse→dedup insert→merchant-rule categorise; regex-first, LLM tracker stays as fallback). (P3) 6-hourly APScheduler ingest job + `POST /api/areas/finance/ingest/run`. (P4) sub-routers `finance_payables.py` (`/payables` monthly checklist unifying bills+EMIs+CC dues+paid state, `/payables/pay` toggle, `/cc-bills` CRUD) and `finance_rules.py` (merchant-rule CRUD); `investments/summary` now returns `committed_monthly`. (P5) Frontend `PayablesTab` (checklist w/ optimistic paid toggle + month nav + KPIs), `RulesTab`, Inbox "Sync emails" button; `tsc --noEmit` + `vite build` clean. (P6) owner-only monthly finance summary → vault (`vault_summary.py`, allowlisted `01-finance/monthly-summary.md`). (P7) monthly CSV backup of all 15 finance tables (`backup.py`). Verified end-to-end on SQLite: payables aggregation (bill+EMI+CC = correct total), paid-toggle persistence, and full vault-summary rendering (cashflow/categories/budgets/payables checklist/EMIs/committed-vs-actual/accounts).
- Blockers: No DB/Docker in this sandbox → migration not yet applied and full pytest not run here (parser logic validated directly; app import blocked only by the tiktoken proxy issue, unrelated to changes). Parser regexes use realistic **synthetic** fixtures — need Utsav's real sample emails to pin to live templates.
- Next: In Docker — `alembic upgrade head`, `restart backend`, `uv run pytest tests/test_email_parsers.py -q`, connect Gmail, run ingest, walk review→approve→ledger/payables/vault. Then swap real email fixtures into the parser tests. See `docs/FINANCE_OS_PLAN.md`.

## 2026-07-13 — claude-code (layout centralization + code review)
- Shipped: **Theme/spacing centralization + 5 bug fixes.** (1) Removed `spacing: appSpacing` from `buildTheme()` in `aiosTheme.ts` — this was overriding the DS 4pt grid with a 12pt scale across all 121 `@ledgr/ui` internal spacing usages, causing 3× gaps on GoalsPage and broken scroll. The 12pt scale now lives in `frontend/src/theme/layout.ts` as a standalone `spacing` export. (2) Migrated `TopBar`, `BottomNav`, `AreaSettingsPage`, `Sidebar`, `AppShell`, `WorkspaceLayout` hardcoded px values to `layout.ts` constants (`TOPBAR_HEIGHT`, `BOTTOM_NAV_HEIGHT`, `SIDEBAR_NAV_WIDTH`, `SIDEBAR_NAV_COLLAPSED_WIDTH`, `SETTINGS_RAIL_WIDTH`). (3) Code review (high effort): fixed `WorkspaceLayout Main` gap `16px→24px` (CLAUDE.md mandates 24px), fixed `AppShell ContentArea padding-bottom: 72px` → `${BOTTOM_NAV_HEIGHT}` (orphaned after layout.ts migration), removed `// @ts-nocheck` from `AreaTabs.tsx`. (4) Polish: `setTimeout(10)→requestAnimationFrame` in `AssistantChatInput.handleMention`, deprecated `substr(2,9)→slice(2,11)` (×2 in same file). `tsc --noEmit` clean throughout.
- Blockers: File deletion still blocked — 7 stale files user approved but classifier blocked `rm`: `backend/test_chat.py`, `frontend/update_progress.py`, `design-system/aios-web/` folder, `lessons.md`, `SAAS_IMPLEMENTATION_PLAN.md`, `docs/SHIP_READINESS_AUDIT.md`, `docs/DESIGN_SYSTEM_AUDIT.md`. Delete manually.
